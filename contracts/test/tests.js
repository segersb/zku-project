...
describe("Polls", function () {
  const UNIT_PRICE = 1_100_000_000_00_000
  let poseidon
  let polls
  let pollId
  let cid
  ...
  let snapshotLeaf4
  let snapshotTree
  let token1Vote1Proof
  let token1Vote2Proof
  let token2Vote1Proof

  before(async function () {
    this.timeout(60000000)
    poseidon = await buildPoseidon()

    pollId = '0xd9aca7cc64c54031a2bdacf31f5b7673'
    cid = 'bafkreian266thfymaekepwnaetwritbqlifuslhiadd4ygvfytv6krr2na'
    collection = '0x9378368ba6b85c1fba5b131b530f5f5bedf21a18'
  ...
  it("Create a poll", async function () {
    ...
    await polls.createPoll(pollId, cid, 4, snapshotTree.root, 2, {
      value: UNIT_PRICE * 4
    })
    assert.equal(await polls.getPollCid(pollId), cid)
  })

  it("Creating an poll with an existing ID fails", async function () {
    this.timeout(60000000)
    ...
    assert.isNotEmpty(error, 'Creating a poll with an existing ID should fail')
    assert.include(error, 'DuplicatePoll', 'A DuplicatePoll error should be thrown')
  })

  it("Vote for a poll", async function () {
    this.timeout(60000000)
    ...
    await polls.vote(token1Vote1Proof.a, token1Vote1Proof.b, token1Vote1Proof.c, token1Vote1Proof.input)
    await polls.vote(token2Vote1Proof.a, token2Vote1Proof.b, token2Vote1Proof.c, token2Vote1Proof.input)
    assert.equal(await polls.getPollResult(pollId, 1), 2)
    assert.equal(await polls.getPollResult(pollId, 2), 0)
  })

  it("Voting twice for a poll fails", async function () {
    this.timeout(60000000)
    ...
    let error = "";
    try {
      await polls.vote(token1Vote2Proof.a, token1Vote2Proof.b, token1Vote2Proof.c, token1Vote2Proof.input)
    } catch (e) {
      error = e.toString()
    }

    assert.isNotEmpty(error, 'Voting twice for a poll should fail')
    assert.include(error, 'DuplicateVote', 'A DuplicateVote error should be thrown')
  })

  it("Voting for an unknown poll fails", async function () {
    this.timeout(60000000)
    ...
    assert.isNotEmpty(error, 'Voting for an unknown poll should fail')
    assert.include(error, 'UnknownPoll', 'An UnknownPoll error should be thrown')
  })

  it("Voting with a proof for a wrong snapshot fails", async function () {
    this.timeout(60000000)

    await polls.createPoll(pollId, cid, 4, 1, 2, {
      value: UNIT_PRICE * 4
    })
    ...
    assert.isNotEmpty(error, 'Voting with a proof for a wrong snapshot should fail')
    assert.include(error, 'InvalidSnapshot', 'A InvalidSnapshot error should be thrown')
  })

  it("Voting with an invalid proof fails", async function () {
    this.timeout(60000000)
    ...
    let error = "";
    try {
      await polls.vote(token1Vote1Proof.c, token1Vote1Proof.b, token1Vote1Proof.a, token1Vote1Proof.input)
    } catch (e) {
      error = e.toString()
    }

    assert.isNotEmpty(error, 'Voting with an invalid proof should fail')
    assert.include(error, 'InvalidProof', 'A InvalidProof error should be thrown')
  })

  it("Voting when the poll is full fails", async function () {
    this.timeout(60000000)

    await polls.createPoll(pollId, cid, 1, snapshotTree.root, 2, {
      value: UNIT_PRICE
    })
    ...
    assert.isNotEmpty(error, 'Voting when the poll is full should fail')
    assert.include(error, 'PollFull', 'A PollFull error should be thrown')
  })
})